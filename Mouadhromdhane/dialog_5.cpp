    ...
    ui(new Ui::Dialog_5)
{
    ...
    qry2.first();
                 ...
                 QSqlQuery qry_1;
                 qry_1.prepare("SELECT sum(VALEUR) FROM GESTIONREV WHERE (extract(month from ID))  ="
                               " extract(month from :ID) AND (extract(YEAR from ID))  = extract(YEAR from :ID) AND TYPE= 'COMMITIONS_TRANSACTIONS';");
                 ...
                 qDebug()<<car;
                 QSqlQuery qry_2;
                 qry_2.prepare("SELECT sum(VALEUR) FROM GESTIONREV WHERE (extract(month from ID))  ="
                               " extract(month from :ID) AND (extract(YEAR from ID))  = extract(YEAR from :ID) AND TYPE= 'LIVRAISONS';");
                 ...
                 qDebug()<<ele;
                 QSqlQuery qry_3;
                 qry_3.prepare("SELECT sum(VALEUR) FROM GESTIONREV WHERE (extract(month from ID))  ="
                               " extract(month from :ID) AND (extract(YEAR from ID))  = extract(YEAR from :ID) AND TYPE= 'SERVICES_EN_LIGNE';");
                 ...
                 qDebug()<<com;
                 QSqlQuery qry_4;
                 qry_4.prepare("SELECT sum(VALEUR) FROM GESTIONREV WHERE (extract(month from ID))  ="
                               " extract(month from :ID) AND (extract(YEAR from ID))  = extract(YEAR from :ID) AND TYPE= 'TRANSACTION_POSTALE';");
