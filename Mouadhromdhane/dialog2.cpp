...
{
    ui->setupUi(this);
    QString titre = "Comparaisons Par Mois";
...
QSqlQuery qry2;
qry2.prepare("SELECT * FROM GESTIONDEP WHERE (extract(month from DATE_ENR))  = extract(month from :ID2) AND (extract(YEAR from DATE_ENR))  = extract(YEAR from :ID2);");
...
QSqlQuery qry4;
qry4.prepare("SELECT sum(VALEUR) FROM GESTIONDEP WHERE (extract(month from DATE_ENR))  = extract(month from :ID2) AND (extract(YEAR from DATE_ENR))  = extract(YEAR from :ID2);");
...
//model->setQuery("SELECT * FROM GESTIONDEP  WHERE DATE_ENR BETWEEN :ID ANDqry.bindValue(":Date",id2); :Date;");
